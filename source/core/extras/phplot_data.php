<?php
/* $Id: phplot_data.php,v 1.2 2004-05-04 15:23:45 alex Exp $
 * 
    ...
    function DoScaleData($even, $show_in_legend) 
    {
        ...
        for ($i=$offset; $i < $end; $i++) {
            if ($max[$i] == 0 || $max[$i] == $maxmax) {
                ...
                if ($even) {
                    $innomatic = pow(10,round(log10($maxmax / $max[$i]))-1);
                    if ($innomatic * $max[$i] * 5 < $maxmax) {
                        $innomatic *= 5;
                    } elseif ($innomatic * $max[$i] * 2 < $maxmax) {
                        $innomatic *= 2;
                    }
                } else {
                    $innomatic = $maxmax / $max[$i];
                    $digits = floor(log10($innomatic));
                    $innomatic = round($innomatic/pow(10,$digits-1))*pow(10,$digits-1);
                }
                $amplify[$i] = $innomatic;
            }
